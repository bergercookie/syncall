...
try:
    from syncall.cattr.cattr_side import CattrSide
    from syncall.taskwarrior.taskwarrior_side import TaskWarriorSide
except ImportError:
    inform_about_app_extras(["Cattr", "tw"])
...
from syncall.cli import  opts_cattr,opts_miscellaneous, opts_tw_filtering
from syncall.tw_cattr_utils import convert_cattr_to_tw, convert_tw_to_cattr


# CLI parsing ---------------------------------------------------------------------------------
@click.command()
@opts_cattr(hidden_gid=False)
@opts_tw_filtering()
@opts_miscellaneous("TW", "Cattr")
def main(  # noqa: PLR0915, C901, PLR0912
    ...
    """Synchronize your tasks in Cattr with filters from Taskwarrior."""
  ...
  #  )
    ...
    if combination_name is not None:
        ...
        cattr_task_gid = app_config["cattr_task_gid"]
    # combination manually specified ----------------------------------------------------------
        ...
        combination_name = cache_or_reuse_cached_combination(
            config_args={
                ...
                "tw_tags": tw_tags,
                "cattr_task_gid": cattr_task_gid,
            },
            config_fname="tw_cattr_configs",
    ...
    with Aggregator(
        side_A=cattr_side,
        side_B=tw_side,
        converter_A_to_B=convert_cattr_to_tw,
        converter_B_to_A=convert_tw_to_cattr,
        resolution_strategy=get_resolution_strategy(
            resolution_strategy,
            side_A_type=type(cattr_side),
